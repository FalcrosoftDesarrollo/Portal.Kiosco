...
namespace Portal.Kiosco.Properties.Views
{
    ...
    //            contenedorImagen.Child = CargarImagen();
    ...
    public partial class SeleccionarFuncion : Window
    {
        ...
        public SeleccionarFuncion()
        {
            ...
            lblDuracion.Content = "Duración: " + App.Pelicula.Duracion + " min";
            lblGenero.Content = "Genero: " + App.Pelicula.Genero;
            var idioma = App.Pelicula.Idioma;
            lblIdioma.Content = "Idioma: " + idioma;
            //App.IsFecha = false;
            if (App.ob_diclst.Count > 0)
            {
                lblnombre.Content = "!HOLA " + App.ob_diclst["Nombre"].ToString() + " " + App.ob_diclst["Apellido"].ToString() + "!";
        ...
        private async void btnSiguiente_Click(object sender, RoutedEventArgs e)
        {
            ...
            transicion.Show();
            this.Close();
            var openWindows = new LayoutAsientos(config);
            await openWindows.LoadDataAsync();
            openWindows.Show();
            transicion.Close();
        }
            ...
            try
            {
                ...
                foreach (var pelicula in listpelicula)
                {
                    ...
                    foreach (var fechas in pelicula.DiasDisponibles.Where(x => x.fecunv == fechasel).OrderByDescending(or => or.fecunv))
                    {
                        ...
                        if (App.version == pelicula.Version)
                        {
        ...
        public async Task CalcularTarifaAsync()
        {
            var tituloriginal = App.Pelicula.TituloOriginal;
            var fechaSel = App.Pelicula.FechaSel.Substring(3);
            var horaSel = App.Pelicula.HoraSel;
            ...
            lc_srvpar = ob_fncgrl.EncryptStringAES(lc_srvpar);
            var lc_result = await Task.Run(() => ob_fncgrl.WebServices($"{App.ScoreServices}scocar/", lc_srvpar));
                    ...
                    if (App.version == pelicula.Version)
                    {
                        ...
                        foreach (var fecha in pelicula.DiasDisponibles.Distinct())
                        {
                            ...
                            if (contador == 0)
                            {
                                ...
                                if (lc_result.StartsWith("0"))
                                {
                                    ProcesarResultado(lc_result, formato, fecha.fecunv, pelicula.Version);
                                }
                            }
                            contador++;
                        }
        ...
        private void ProcesarResultado(string lc_result, string formato, string fechaUnv, string version)
        {
            lc_result = lc_result.Replace("0-", "");
            var ob_diclst = JsonConvert.DeserializeObject<Dictionary<string, object>>(lc_result);
            var ob_lsala = JsonConvert.DeserializeObject<Dictionary<string, object>>(ob_diclst["GetHora"].ToString());
            ...
            if (Zonas != null && Zonas.Count > 0 && ob_lisprg != null)
            {
                foreach (var item in ob_lisprg)
                {
                    foreach (var itemZonas in Zonas)
                    {
                        ProcesarZonas(item, itemZonas, formato, fechaUnv, version);
                    }
                }
            }
        }

        private void ProcesarZonas(sala item, KeyValuePair<string, string> itemZonas, string formato, string fechaUnv, string version)
        {
            if (item.hora != null && item.hora.Count > 0)
            {
                foreach (var item2 in item.hora.OrderBy(x => x.horario))
        ...
        private Dictionary<string, int> filaContenedores = new Dictionary<string, int>
{
    { "GENERAL3D", 0 },
    { "EDBlackStar", 0 },
    { "SUPERNOVA", 0 },
    ...
    { "P4DX3D", 0 }
        ...
        private void AgregarBotonesPorFormato(dynamic item3, string horario, string militar, string formato, string zonaKey)
        {
            ...
            if (!horasProcesadasPorSala[tipoSalaKey].Contains(horario))
            {
                ...
                {
                    case "GENERAL3D":
                        border = CrearBotonHora(horario, militar, "GENERAL3D");
                        ContenedorHoras3D.Children.Add(border);
                        filaContenedores["GENERAL3D"]++;
                        break;
                    case "EDBlackStar":
                        border = CrearBotonHora(horario, militar, "EDBlackStar");
                        ContenedorHoras3DBlackStar.Children.Add(border);
                        filaContenedores["EDBlackStar"]++;
                        break;
                    case "SUPERNOVA":
                        border = CrearBotonHora(horario, militar, "SUPERNOVA");
                        ContenedorHoras3DSuperNova.Children.Add(border);
                        filaContenedores["SUPERNOVA"]++;
                        break;
                    case "P4DX":
                        border = CrearBotonHora(horario, militar, "P4DX");
                        ContenedorHoras4DX.Children.Add(border);
                        filaContenedores["P4DX"]++;
                        break;
                    case "GENERAL":
                        border = CrearBotonHora(horario, militar, zonaKey);
                        ContenedorHorasGeneral.Children.Add(border);
                        filaContenedores["GENERAL"]++;
                        break;
                    case "BlackStar":
                        border = CrearBotonHora(horario, militar, "BlackStar");
                        ContenedorHorasBlackStar.Children.Add(border);
                        filaContenedores["BlackStar"]++;
                        break;
                    case "Supernova":
                        border = CrearBotonHora(horario, militar, "Supernova");
                        ContenedorHorasSupernova.Children.Add(border);
                        filaContenedores["Supernova"]++;
                        break;
                    case "P4DX3D":
                        border = CrearBotonHora(horario, militar, "P4DX3D");
                        ContenedorHoras3D4DX.Children.Add(border);
                        filaContenedores["P4DX3D"]++;
                        break;
                }
        ...
        private string GetTipoSalaKey(string tipoSala, string formato)
        {
            if (formato == "Digital 3D")
            {
                Formato3D.Visibility = Visibility.Visible;
                if (tipoSala.Contains("GENERAL"))
                    return "GENERAL3D";
                if (tipoSala.Contains("BLACK STAR 3D"))
                    return "EDBlackStar";
            ...
            if (formato == "Digital 2D")
            {
                Formato2D.Visibility = Visibility.Visible;
                if (tipoSala.Contains("GENERAL"))
                    return "GENERAL";
                if (tipoSala.Contains("BLACK STAR"))
                    return "BlackStar";
        ...
        private void CrearBotonFecha(string fecha, string fecunv)
        {
            ...
            border.BorderThickness = new Thickness(1);
            border.CornerRadius = new CornerRadius(10);
            border.Margin = new Thickness(6, 6, 6, 6);
            border.Child = btn;
        ...
        private Border CrearBotonHora(string hora, string idFuncion, string tipotarifa)
        {
            ...
            border.Background = Brushes.White;
            border.CornerRadius = new CornerRadius(5);
            border.Margin = new Thickness(0, 7, 6, 5);
            border.BorderThickness = new Thickness(1);
            border.BorderBrush = Brushes.Red;
            border.Child = btn;
            border.Name = tipotarifa.Replace(" ", "_");
        ...
        private async void btnSeleccionHora_Click(object sender, RoutedEventArgs e)
        {
            Button clickedButton = sender as Button;
        ...
        private void DOBLADA3D_MouseLeftButtonDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            Border clickedBorder = sender as Border;
            App.version = "DOBLADA";
            HandleBorderSelection(clickedBorder);
        }

        private void SUBTITULADA3D_MouseLeftButtonDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            App.version = "SUBTITULADA";
            Border clickedBorder = sender as Border;
            HandleBorderSelection(clickedBorder);
        }

        private void DOBLADA2D_MouseLeftButtonDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            App.version = "DOBLADA";
            Border clickedBorder = sender as Border;
            HandleBorderSelection(clickedBorder);
        ...
        private void SUBTITULADA2D_MouseLeftButtonDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            App.version = "SUBTITULADA";
            Border clickedBorder = sender as Border;
            HandleBorderSelection(clickedBorder);
        ...
        private void HandleBorderSelection(Border clickedBorder)
        {
            if (_previouslySelectedBorder != null && _previouslySelectedBorder != clickedBorder)
            {
                // Reset previous border
                _previouslySelectedBorder.Background = Brushes.Transparent;
                UpdateLabelForeground(_previouslySelectedBorder, Brushes.Black);
            }

            // Update current border
            clickedBorder.Background = new SolidColorBrush(ColorConverter.ConvertFromString("#F30613") as Color? ?? Colors.Red);
            UpdateLabelForeground(clickedBorder, Brushes.White);

            _previouslySelectedBorder = clickedBorder;

            CargarFechasDesdeSelect();
        }

        private void UpdateLabelForeground(Border border, Brush color)
        {
        ...
        private void SelectInitialBorders()
        {
            // Selecciona el borde "2D Doblada" y "3D Doblada" al inicio
            var border2DDoblada = FindName("DOBLADA2D") as Border;
            var border3DDoblada = FindName("DOBLADA3D") as Border;

            if (border2DDoblada != null)
            {
                UpdateBorderSelection(border2DDoblada);
            }

            if (border3DDoblada != null)
            {
                UpdateBorderSelection(border3DDoblada);
            }
        }

        private void UpdateBorderSelection(Border clickedBorder)
        {
             
            clickedBorder.Background = new SolidColorBrush(ColorConverter.ConvertFromString("#F30613") as Color? ?? Colors.Red);
            Label currentLabel = clickedBorder.Child as Label;
